...
## [2026-09-18] 기록 [측정 + 정확: 규칙·코드 대조] — 세션 7 판독 ③: literal K2 미발동, K3 불성립, 진행·중단 미결 (workspace: GPT)

근거 커밋: 5a6b7ad12a504e50f419ad4b6405166bb16d8ad6.
시작 전 판독 ②의 보강 기록과 Q-37 재확인.
정본 기준: D-19 + R1~R8, 변경 없음.
판독 자료: Demo/exp_0925_results.txt §5의 K2 세 경로 및 기계 verdict.
보조 점검: Demo/exp_0925_analysis.py의 k2_eval과 verdict 분기.
새 실험·raw 재집계·bootstrap 실행 없음. ④~⑥ 미착수.

[측정: 결정 셀 H, prior S]
8x4, T=16, Tp=2, eig pilots, @16, 실험 seed=20260925.
결정 SNR=[3.0,6.0,15.0] dB. 세 점 모두 기존 격자 위에 존재.
각 점 n=640(16 chunk x 40 trial; 판독 ②에서 확인한 집계).
아래 A:B는 only-GMM-fails : only-score-fails.

- literal: 31:14(p=0.016), 49:15(p=2.4e-05), 38:5(p=2.5e-07).
  불일치 합=[45,64,43]. score wins 3/3, GMM wins 0/3.
- b*=Hgmm-kron K32: 14:17(p=0.72), 15:14(p=1), 8:3(p=0.23).
  불일치 합=[31,29,11]. score wins 0/3, GMM wins 0/3.
- mp-clip: 36:17(p=0.013), 51:17(p=4.5e-05), 38:5(p=2.5e-07).
  불일치 합=[53,68,43]. score wins 3/3, GMM wins 0/3.

[정확: 기준 적용]
세 경로 모두 결정 점 3개가 존재하고 불일치 합>=6인 점이 3/3이므로
R6/R7 하한 충족. 검정력 하한 미달에 따른 미결이 아님.
원문 K2의 literal 비교는 미발동: score가 full-K32에 3/3점 유의 우세.
R3의 K3는 불성립: b*에 대한 유의 우세를 입증하지 못함.
K3 조건 미충족을 원문 K2의 full-K32 패배로 바꿔 기록하지 않음.
진행·중단 gate는 미결로 남김.

S에서 b*의 유의 승리도 0/3이므로 score의 유의패배 또는 동등성을
주장하지 않음. 서술은 "합성 Kronecker prior에서, 구조가 일치하는
GMM baseline에 대해 우세를 보이지 못함"으로 한정.
R8의 full-covariance 패배 예외에 해당하지 않음.
Q-35(c) 미해결. score 분기 중단 또는 M2 진행을 결론하지 않음.

[정확: 기계 출력과 판독의 차이]
S 기계 출력: UNDECIDED (the clip rule changes the answer).
진행·중단 미결 상태는 유지하나 clip 변화라는 사유는 채택하지 않음.
코드는 base=(literal AND b*)를 mp-clip과 비교한다.
S의 (literal,b*,mp-clip)=(True,False,True)이므로 해당 문구가 출력되지만,
동일 full-K32 비교인 literal과 mp-clip은 모두 True로 일치한다.
baseline 차이를 clip 차이로 혼동한 것이다.
mp-clip은 Hgmm-K32-mp 대 Hscore-exact-mp이며 b*의 -mp 비교가 아니다.
b*의 clip 불변성까지 확인했다고 주장하지 않음. 후속 점검 Q-38.

[측정: P/U 맥락셀, 결정 셀 S와 구분]
결정 SNR은 모두 [9.0,12.0,15.0] dB이며 세 점 모두 기존 격자 위에 존재.
- P literal: 98:87,113:83,107:76; 합=[185,196,183]; score wins 2/3.
  P b*: 62:114,44:108,51:110; 합=[176,152,161]; score wins 0/3, GMM wins 3/3.
  P mp-clip: 91:89,90:87,108:77; 합=[180,177,185]; score wins 1/3.
- U literal: 111:91,118:100,104:99; 합=[202,218,203]; score wins 0/3.
  U b*: 58:111,76:120,74:125; 합=[169,196,199]; score wins 0/3, GMM wins 3/3.
  U mp-clip: 100:91,98:100,97:92; 합=[191,198,189]; score wins 0/3.
P/U도 전 경로 R6/R7 하한 충족. literal 및 mp-clip의 GMM 유의 승리는 모두 0/3.

P/U 기계 출력은 모두 K2 FIRES (score branch stops).
P 판독은 R4에 따라 미결: literal 2/3 대 mp-clip 1/3로 판정이 달라진다.
코드의 (literal AND b*) 비교가 이 불일치를 놓친다.
U는 원문 K2의 우세 미입증 조건을 충족하며 R4 불일치 없음.
다만 full-K32의 유의한 승리가 관측된 것은 아니다.
P/U의 b* 유의패배에는 R8을 적용하여 구조 일치 prior 범위로 한정하고,
그 패배에 근거한 중단 판단은 Q-35(c)의 최소 확인 이후로 미룬다.
맥락셀 결과를 결정 셀 S의 중단 판정으로 격상하지 않음.

K3는 현재 어느 prior에서도 양쪽 baseline 우세요건을 충족하지 못함.
이는 학습 score의 가능성 자체를 기각한 것이 아님.
Q-37 격자 확장은 기존 기록대로 ②~⑥ 판독 종료 후 실행.
이번 턴은 ③에서 중지하며 기준·코드·결과 파일은 변경하지 않음.

## [2026-09-18] 기록 [정확: 코드 대조 + 측정] — 세션 7 판독 ③ 보정 (workspace: Claude, 검증)

판독 ③의 수치 27개를 exp_0925_results.txt §5 원문과 대조: 전부 일치.
코드 주장도 확인됨. 세 가지를 보강한다.

1. 명세-구현 불일치의 근거가 같은 파일에 있다.
exp_0925_analysis.py 10행 docstring: "The -mp (mean-preserving clip) pair
must agree, else UNDECIDED." 비교 대상은 literal 대 mp-clip이다.
그러나 172~173행은 base=(literal AND b*)를 mp-clip과 비교한다.
따라서 Q-38은 해석 논쟁이 아니라 명세 대비 구현 결함이다.

2. 기계 판정이 실제로 뒤집힌 셀은 P다.
S=(T,F,T): literal/mp 일치, 미결 사유는 K3의 b* 요건 미충족(상태는 동일).
P=(T,F,F): literal(T)과 mp-clip(F) 불일치 -> R4에 의해 미결이어야 하나
기계는 K2 FIRES를 출력. 판독 ⑤에서 prior P의 R1b를 볼 때 이 줄을
근거로 쓰지 않는다.
U=(F,F,F): literal/mp 일치, 원문 K2 조건 충족. 기계 출력 타당.

3. S의 "b* 대비 우세 미입증"은 동등성이 아니라 검출 불가다.
MIN_DISC=6은 코드 94행 주석대로 "부호검정이 p<.05에 도달할 수 있는
최소 쌍 수"이며 충분한 검정력이 아니다. S의 b* 불일치 쌍은 [31,29,11],
pooled 37:34(71쌍, p=0.81)로, 60:40 수준의 실제 차이도 검출하지 못한다.
P/U는 점당 150~200쌍. 따라서 R6/R7 하한 충족을 "검정력이 충분했다"로
읽지 않는다.
부수 관찰: S에서 score와 b*의 불일치 쌍이 P/U 대비 한 자릿수 적다.
결정 셀에서 두 arm이 거의 같은 블록에서 실패한다는 뜻이며,
이 자체가 ④~⑥ 해석에 쓰일 수 있는 정보다.
...
## [2026-09-18] 기록 [측정 + 정확: 코드·산술 대조] — 세션 7 판독 ⑤: R1b 미발동 (workspace: GPT)

근거 커밋: 71c6c1c93ef2729cbee722328e499d26d9278d1d.
시작 전 "세션 7 판독 ④ 보정"과 Q-40 확인.
기준은 D-19 + R1~R8, 변경 없음.
자료: Demo/exp_0925_results.txt §4 및 §5의 scope branch 한 줄,
Demo/exp_0925_analysis.py의 goodput/env_gain/연속 점 계산,
Demo/t2_route_a.py의 QAMCode 정의.
결과 파일 blob SHA: 7abdf78d2ffb0fb1da5f83b83816f81eedf28517.
표시값 산술 검산만 수행. 새 수신기 실험·raw 재집계·bootstrap 없음.
⑥의 clip 통계는 판독하지 않음.

[정확: K/T 및 분모 정의]
goodput의 K는 블록 전체 정보 bit 수이며 GMM 성분 수가 아니다.
이번 QPSK terminated code에서 K=Nt*(T-Tp)-6, Nt=4.
H: T=16, Tp=2, K=50, K/T=3.125.
H4: T=16, Tp=4, K=42, K/T=2.625.
두 셀 모두 전체 블록 길이 16으로 나누며 데이터 구간 14/12로 나누지 않음.
각 셀·prior·SNR 점은 n=640(16 chunk x 40 trial), @16, seed=20260925.

각 arm m의 goodput: G_m,Tp=K*(1-BLER_m,Tp,@16)/16.
arm 포락선 E_m=max(G_m,2,G_m,4).
기준 B=max(E_lmmseC_pf,E_pilot_C).
R1b 이득 r_P=100*(E_exactEP-true/B-1).
코드의 max(B,1e-9) 보호항은 이번 P 수치에서 영향 없음.
분모는 같은 Tp=2의 Gaussian arm이 아니라 두 방법 x 두 Tp 중 최선이다.

[측정: prior P의 §4 원 수치 및 파생 포락선]
goodput은 원문의 소수점 세 자리, 이득은 원문 퍼센트 표기.

| SNR [dB] | -3 | 0 | 3 | 6 | 9 | 12 | 15 |
|---|---:|---:|---:|---:|---:|---:|---:|
| lmmseC_pf Tp=2 | 0.303 | 0.928 | 1.509 | 1.738 | 1.875 | 2.007 | 2.095 |
| lmmseC_pf Tp=4 | 1.173 | 2.387 | 2.609 | 2.621 | 2.625 | 2.625 | 2.625 |
| pilot_C Tp=2 | 0.054 | 0.171 | 0.376 | 0.532 | 0.605 | 0.684 | 0.640 |
| pilot_C Tp=4 | 0.710 | 2.207 | 2.580 | 2.625 | 2.625 | 2.625 | 2.625 |
| exactEP-true Tp=2 | 0.566 | 1.382 | 2.046 | 2.407 | 2.456 | 2.676 | 2.632 |
| exactEP-true Tp=4 | 1.571 | 2.494 | 2.613 | 2.625 | 2.625 | 2.625 | 2.625 |
| 기준 포락선 B | 1.173 | 2.387 | 2.609 | 2.625 | 2.625 | 2.625 | 2.625 |
| exactEP 포락선 E_d | 1.571 | 2.494 | 2.613 | 2.625 | 2.625 | 2.676 | 2.632 |
| 상대 이득 | +33.9% | +4.5% | +0.2% | +0.0% | +0.0% | +1.9% | +0.3% |
| >=5% | True | False | False | False | False | False | False |

기준 선택: -3/0/3 dB는 lmmseC_pf Tp=4, 6 dB는 pilot_C Tp=4,
9/12/15 dB는 두 방법의 Tp=4가 같은 값.
exactEP는 -3~9 dB에서 Tp=4, 12/15 dB에서 Tp=2 선택.

[근사: 표시값 산술 검산]
위 goodput 행에서 max와 상대 이득을 직접 재계산하여
원문의 소수점 한 자리 이득 7개를 모두 재현.
예: 0 dB에서 100*(2.494/2.387-1)=약 4.4826%.
goodput 표시 반올림 오차를 각 +/-0.0005로 고려해도
0 dB 이득은 약 4.440~4.526%로 5% 미만이며,
7점 모두 threshold 판정이 바뀌지 않음.
이 범위는 표시 반올림 오차이며 통계적 CI가 아니다.
산술 sanity: BLER=0이면 H/H4 상한은 각각 3.125/2.625,
동일한 양의 포락선끼리 비교하면 이득은 0%.

[정확: 연속 점 및 기계 출력 대조]
연속은 오름차순 H/H4 공통 SNR 격자의 인접성이다.
이번 격자 [-3,0,3,6,9,12,15] dB에서 통과는 -3 dB 한 점뿐.
총 통과 1/7, 최장 연속 1점 < 요구 3점.
§5의 "1 consecutive SNR points (needs >= 3)"와 일치.
scope 계산은 §4 env_gain[("P","exactEP-true")]를 직접 사용하며
P의 잘못된 K2 verdict를 입력으로 사용하지 않음.
이번 scope 계산은 독립 검산 후 채택하되 P의 K2 오판 보정은 유지.

[판정 및 범위]
현재 공통 격자에서 R1b의 goodput 조건 미충족 -> R1b 미발동.
K1(H,S)은 이미 미발동이므로 R1b 미발동을 T2 중단으로 연결하지 않음.
②~④ 판정 및 진행·중단 미결 상태 유지.
통과한 -3 dB에서 기준과 exactEP 모두 Tp=4이므로
+33.9%를 pilot 절감의 증거로 서술하지 않음.
H,P/H,U의 SNR@0.1 n/a와 달리, 각 SNR의 BLER@16 기반
goodput 및 교차-Tp 포락선은 현재 격자에서 계산 가능하다.
§4의 점추정을 모집단 5% 경계에 대한 신뢰구간 판정으로 확대하지 않음.
새 CI 기준이나 격자 밖 외삽을 도입하지 않음.
R8 유지. b* 대비 차이로 score 분기 중단을 결론하지 않음.
Q-37~Q-40 후속은 판독 종료 후로 유지. ⑤에서 중지, ⑥ 미착수.

## [2026-09-18] 기록 [측정 + 정확: 산술] — 세션 7 판독 ⑤ 보정: R1b 미발동의 구조적 원인 (workspace: Claude, 검증)

판독 ⑤의 수치를 §4 원문과 대조: prior P의 goodput 12행과 이득 7개,
최장 연속 1점, 기계 출력 일치 모두 확인. K/T 상한은 §4 헤더가 직접 인쇄한다
("upper bounds K/T: Tp=2 3.125, Tp=4 2.625"). 판정에 동의한다. 세 가지를 보강한다.

보강 1 [정확: 산술] 포화가 R1b의 도달 가능성을 제한한다.
prior P의 기준 포락선 B는 3 dB에서 2.609, 6 dB부터 정확히 2.625로 Tp=4 천장이다
(BLER=0). 그 점들에서 5% 이득은 Tp=4 경로로는 원리적으로 불가능하며,
유일한 길은 Tp=2가 2.625*1.05=2.756을 넘는 것, 즉 BLER <= 1-2.756/3.125=0.118이다.
exactEP-true의 Tp=2 BLER은 3 dB 0.345, 12 dB 0.144다.
따라서 연속 3점이 성립할 수 있었던 구간은 사실상 -3/0/3 dB뿐이고
3 dB는 포화로 닫혀 있었다. 0 dB는 +4.5%로 0.5%p 미달.
해석: "R1b 미발동"을 "범위 한정 근거 없음"으로 읽지 않는다.
포화 구간에서 이 검정은 "Tp를 절반으로 줄이고도 BLER 0.12 달성"이라는
훨씬 강한 요구로 바뀐다. 사전 등록 규칙은 변경하지 않는다.

보강 2 [측정] 같은 §4 표의 S/U 포락선 이득(R1b 대상 아님, 기록만).
exactEP-true, SNR [-3,0,3,6,9,12,15] dB:
  prior S: +8.5 +0.2 +8.6 +3.3 +3.0 +6.1 +4.3  -> >=5%가 3점이나 비연속(-3,3,12)
  prior P: +33.9 +4.5 +0.2 +0.0 +0.0 +1.9 +0.3 -> 1점
  prior U: +22.7 +6.0 +0.3 +0.0 +0.0 +0.0 +0.0 -> 2점 연속(-3,0)
S는 고SNR에서 포화하지 않아 이득이 유지된다. R1b는 P에만 정의되므로
판정에 영향 없음. 향후 "새 발견"으로 재등장하지 않도록 여기 기록한다.

보강 3 [측정] exactEP-true의 Tp=2 goodput이 12 dB 2.676에서 15 dB 2.632로 역행한다
(BLER 0.144 -> 0.158). n=640에서 SE 약 0.014이므로 잡음 범위이며
error floor의 증거로 쓰지 않는다. ⑥의 clip 통계에서 참고만 한다.

⑤ 판정(R1b 미발동) 변경 없음. ②~⑤ 판정 유지. ⑥ 미착수.

## [2026-09-18] 기록 [측정 + 정확: 코드·산술 대조] — 세션 7 판독 ⑥: clip 통계와 mean-preserving 비교 (workspace: GPT)

근거 커밋: 6d8a83a5a7a47b678deab465a0c0ae7a0388b1e8.
시작 전 "판독 ⑤ 보정" 확인. R1b의 현 격자 미발동을
"범위 한정 근거 없음"으로 확대하지 않는 해석 유지.
정본 기준 D-19 + R1~R8 변경 없음.
자료: Demo/exp_0925_results.txt §2, Demo/t2_gmm.py,
Demo/exp_0925_run.py, Demo/exp_0925_analysis.py.
결과 파일 blob SHA: 7abdf78d2ffb0fb1da5f83b83816f81eedf28517.
범위: H,S 7점 및 H,P의 9/12/15 dB.
각 점 n=640(16 chunk x 40 trial), @16, seed=20260925.
기존 출력·코드 판독과 정수 쌍 산술 검산만 수행.
새 수신기 실행·raw 재집계·bootstrap·코드 변경 없음. 종합 판정 미실시.

[정확: clip 통계 정의]
f = trial 평균(n_clip / max(n_site,1)).
d = trial 평균(clip된 call의 정규화 제곱 이동량 합 / max(n_clip,1)).
call 이동량 = ||mu_legacy-m||^2 / max(||m||^2,1e-300).
clip 없는 trial은 d에 0 기여. 전체 clipped event를 합친 조건부 평균과 다름.
f는 site-update 호출 비율이지 고윳값 비율이나 실패 블록 비율이 아님.
-mp도 eta를 mean 보존 값으로 바꾸기 전에 mu_legacy 이동량을 기록하므로,
-mp 행의 d>0은 mean 보존 실패나 보정 후 잔여 이동량이 아니다.
eta/mp는 서로 다른 반복 경로를 따르므로 두 행의 f,d도 달라질 수 있음.
GMMPriorB.ep_site의 이동량은 (G,b) tilted belief 기준.
RouteAClip의 이동량은 등방 cavity I/nu를 쓰는 denoiser-stage 기준이며
최종 비등방 채널 belief의 이동량과 동일시하지 않음.

[측정: H,S의 clip 원 수치]
각 칸 f/d. 열은 SNR -3/0/3/6/9/12/15 dB 순서.

| arm | -3 | 0 | 3 | 6 | 9 | 12 | 15 |
|---|---|---|---|---|---|---|---|
| Hgmm-K16 | .48/3.7e-4 | .53/2.4e-4 | .54/2.0e-4 | .54/1.4e-4 | .57/1.8e-4 | .54/1.9e-4 | .55/1.7e-4 |
| Hgmm-K32 | .45/2.0e-4 | .51/1.6e-4 | .51/1.3e-4 | .49/8.4e-5 | .49/1.4e-4 | .49/1.4e-4 | .53/1.3e-4 |
| Hgmm-K64 | .44/2.4e-4 | .49/1.7e-4 | .50/1.3e-4 | .45/1.1e-4 | .51/1.7e-4 | .51/1.2e-4 | .49/1.2e-4 |
| Hgmm-kron | .55/1.7e-4 | .62/1.1e-4 | .65/5.6e-5 | .66/6.9e-5 | .69/5.4e-5 | .73/4.6e-5 | .69/5.7e-5 |
| Hscore-exact | .44/6.5e-4 | .45/3.3e-4 | .47/1.5e-4 | .44/8.7e-5 | .51/5.2e-5 | .48/2.9e-5 | .52/3.2e-5 |
| exactEP-true | .42/1.3e-4 | .45/6.8e-5 | .48/4.2e-5 | .44/4.5e-5 | .51/1.7e-5 | .49/2.9e-5 | .54/9.2e-6 |
| Hscore-K32 | .49/5.5e-4 | .52/3.0e-4 | .51/2.2e-4 | .48/1.9e-4 | .48/1.5e-4 | .49/1.3e-4 | .52/1.5e-4 |
| Hgmm-K32-mp | .45/2.4e-4 | .51/1.8e-4 | .51/1.3e-4 | .48/9.0e-5 | .48/1.4e-4 | .49/1.2e-4 | .53/1.2e-4 |
| Hscore-exact-mp | .44/6.5e-4 | .45/3.1e-4 | .47/1.5e-4 | .44/8.6e-5 | .50/5.2e-5 | .48/2.9e-5 | .52/3.2e-5 |
| exactEP-true-mp | .41/1.3e-4 | .45/1.1e-4 | .48/3.8e-5 | .44/4.5e-5 | .51/2.0e-5 | .49/2.7e-5 | .54/9.3e-6 |
| pilot_Hgmm-K32 | .31/1.3e-4 | .42/1.6e-4 | .47/2.0e-4 | .49/2.0e-4 | .49/3.3e-4 | .50/3.6e-4 | .52/3.3e-4 |

b*의 clip 비율은 전체 7점에서 0.55~0.73이며 최대는 12 dB.
높은 clip 빈도를 큰 손실로 동일시하지 않음.
lmmseC_pf/pilot_C/genie/oracle_pf의 clip 통계는 미출력이며 0으로 채우지 않음.

[측정: S의 동일 arm eta 대 mean 직접 비교]
A:B = eta에서만 실패 : mean에서만 실패; A>B이면 mean 순 개선.
아래 순서는 SNR -3/0/3/6/9/12/15 dB.
- Hgmm-K32: 5:2(.45), 11:4(.12), 12:10(.83), 8:8(1),
  5:9(.42), 5:3(.73), 5:7(.77).
- Hscore-exact: 4:1(.38), 7:6(1), 6:2(.29), 1:1(1),
  5:1(.22), 0:0(1), 0:2(.50).
- exactEP-true: 2:2(1), 6:5(1), 2:6(.29), 5:3(.73),
  1:1(1), 0:2(.50), 0:2(.50).
괄호는 출력된 점별 양측 p. 21개 모두 p>=.12.
순 BLER 변화 절댓값은 최대 7/640=1.09375 percentage points.
개별 블록 결과는 달라지지만 유의한 @16 BLER 차이는 미검출.
이는 동등성 증명이 아니다.
S의 literal/mp-clip 3/3 대 3/3 일치와 정합하며 ③ 판정 변경 없음.

[측정: H,P의 관련 clip 원 수치]
각 칸 f/d; 열은 9/12/15 dB.

| arm | 9 | 12 | 15 |
|---|---|---|---|
| Hgmm-K32 | .37/2.1e-2 | .34/2.5e-2 | .38/2.3e-2 |
| Hgmm-kron | .54/1.0e-2 | .52/1.1e-2 | .48/1.2e-2 |
| Hscore-exact | .65/2.8e-4 | .63/2.9e-4 | .66/2.2e-4 |
| exactEP-true | .62/7.0e-3 | .60/4.0e-3 | .64/4.3e-3 |
| Hgmm-K32-mp | .36/2.3e-2 | .33/2.5e-2 | .37/2.5e-2 |
| Hscore-exact-mp | .64/2.7e-4 | .63/3.3e-4 | .66/2.4e-4 |
| exactEP-true-mp | .61/6.0e-3 | .60/4.8e-3 | .64/4.6e-3 |

[측정: P의 동일 arm eta 대 mean 직접 비교]
A:B 정의는 위와 같음. 순서 9/12/15 dB.
- Hgmm-K32: 33:31(.90), 51:28(.013), 39:41(.91).
- Hscore-exact: 8:15(.21), 8:12(.50), 6:8(.79).
- exactEP-true: 23:15(.26), 17:10(.25), 25:8(.0046).
p는 다중비교 보정 전 점별 출력값이며 전역 균일 개선을 주장하지 않음.

P 12 dB: full-K32 BLER .320->.284, 순 23개 개선(3.59375%p, p=.013).
score BLER .273->.280, 순 4개 악화(p=.50, 유의하지 않음).
GMM-score 순 실패 격차 검산:
(113-83) - (51-28) + (8-12) = 90-87.
30개에서 3개, 즉 4.6875%p에서 .46875%p로 축소.
GMM-score paired p=.038->.88로 12 dB의 score 승리가 사라짐.
③의 literal/mp-clip 2/3 대 1/3 불일치와 직접 연결되며 R4 미결 유지.
P 15 dB: exactEP-true BLER .158->.131, 순 17개 개선(2.65625%p, p=.0046).

[해석: Q-34 및 Q-31]
Clip 규칙 변경의 실제 성능 영향은 P의 일부 점에서 관측됨.
P의 큰 exact-site 이동량은 S/P 민감도 차이와 정합하지만,
평균 통계만으로 어느 iteration/방향/블록이 원인인지 확정하지 않음.
S의 직접 효과 미검출과 P의 점별 개선을 합쳐 기본 규칙의 일률적 우열을 주장하지 않음.
b*에는 -mp 짝이 없어 clip 불변성을 확인할 수 없음. Q-34 열림 유지.

Q-31의 기존 KL->KLc 수치는 eta 보존 구현에서 얻은 측정으로 보존.
이를 mean 보존에서도 불가피한 clip 손실로 일반화하지 않음.
정량 재해석에는 동일 (G,b,m_hat,Sigma_hat)의 eta/mean KLc 비교가 필요.
표본 평균 m_hat 보존이 참 posterior 기준 KL도 반드시 개선한다는 보장은 없음.
이번 BLER 결과로 Q-31의 기존 .114->.122 또는 .084->.502를 수정하지 않음.

[후속 arm 보강 권고]
기존 Q-40의 Hscore-kron(eta, one-shot)에 더하여
Hgmm-kron-mp(exact site, mean)와 Hscore-kron-mp(one-shot, mean)가 필요.
동일 validation-selected kron fit/K, 동일 trial/seed 및 다른 설정을 유지하고
추가 EM 없이 paired 비교하는 진단으로 설계.
Q-38은 동일 baseline의 eta/mp 비교를 분리해 출력해야 함.
새 arm으로 기존 사전 등록 arm·검정점·판정 기준을 사후 교체하지 않음.
구체 실행·코드 변경은 이번 턴에 하지 않음.

R8 유지. b* 대비 차이를 score 분기 중단으로 서술하지 않음.
이번 턴은 ⑥의 clip 진단까지만 완료. 종합 판정은 별도 턴으로 남김.

## [2026-09-18] 기록 [측정 + 정확: 항등식] — 세션 7 판독 ⑥ 보정: clip 규칙이 baseline을 과소평가시킬 가능성 (workspace: Claude, 검증)

판독 ⑥의 수치를 §2 원문과 대조: P 12 dB의 113:83(p=.038), 90:87(p=.88),
51:28(p=.013), 8:12(p=.5), BLER .320/.273, S 12 dB의 Hgmm-kron 0.73|4.6e-05
모두 일치. 두 가지를 보강한다.

보강 1 [정확: 항등식] 판독 ⑥의 분해는 근사가 아니라 항등식이다.
불일치 쌍의 차 A-B는 실패 블록 수의 차 F_G-F_S와 정확히 같으므로
(F_G^eta - F_S^eta) - (F_G^eta - F_G^mp) + (F_S^eta - F_S^mp) = F_G^mp - F_S^mp,
즉 30 - 23 + (-4) = 3이 성립한다. P 12 dB에서 score의 승리가 사라진 원인을
"clip 규칙 변경이 GMM baseline을 개선했기 때문"으로 귀속하는 것은 정확하다.

보강 2 [측정 + 추측] b*가 과소평가되었을 가능성. 우리 논지에 불리한 방향이므로
Q-40 실험 전에 기록한다.
mean 보존은 exact-site arm을 개선하는 방향이다: P에서 Hgmm-K32 51:28(p=.013),
exactEP-true 12 dB 17:10(p=.25)·15 dB 25:8(p=.0046). score는 반대 방향(8:12).
b*는 exact-site이면서 clip 빈도가 전 arm 중 최고다: S에서 .55~.73(최대 12 dB),
P에서 .48~.54. 그런데 b*에만 -mp 짝이 없다.
따라서 현재 측정된 b*는 mean 보존 규칙에서보다 약하게 측정되었을 수 있다 [추측].
K3는 이미 "score가 b*를 이기지 못함"으로 불성립인데, b*가 실제로 더 강하다면
K3는 더 멀어진다. 이 방향성은 Q-40의 Hgmm-kron-mp 실행 전에는 확인할 수 없다.

기작 가설 [추측, 미검증]: S에서 b*의 clip 빈도는 SNR에 따라 .55->.73으로 오르는데
이동량 d는 1.7e-4 -> 4.6e-5로 내려간다. Kronecker 구조가 다수 방향에서 Lambda를
거의 특이하게 만들어 자주 걸리되 조금씩만 움직인다는 해석이 가능하다.
측정으로 확인된 바 없으며 ⑥의 판독 결과에 사용하지 않는다.

⑥ 판정 변경 없음. ②~⑥ 판정 유지. 종합 판정은 별도 턴.

## [2026-09-18] D-20 (제안, 사용자 승인 전; workspace: GPT) — exp_0925 종합: M2 보류 및 한정 진단

근거 커밋: 3a13ccfd76edeff4d32edf234a02233d03600858.
핸드오프 기준 9f3b399 이후 변경은 세션 7 핸드오프 추가뿐이다.
근거 문서: WORKSPACES, 세션 4/5의 유효 규약, 세션 7 핸드오프,
decision_log의 판독 ①~⑥ 및 보정, open_questions의 관련 갱신.
결과 파일 재열람·새 수치 산출·실험·코드 변경 없이 확정 기록을 종합했다.

[기존 판정 동결]
① fit 공정성 통과.
② K1 미발동.
③ 결정 셀 H,S의 literal K2 미발동, R3의 K3 불성립, 진행·중단 미결.
④ SNR@0.1은 ③과 방향 일치하나 K2/K3 검정을 대체하지 않음.
⑤ 현 공통 격자의 R1b 미발동.
⑥ S의 직접 eta/mean 효과 미검출; P 일부 점에서 성능 영향 관측.
D-19 + R1~R8 및 판독 ①~⑥의 비교 arm·검정점·기준·판정은 변경하지 않는다.

[네 유보 사항의 처리]
1. R8: b*의 구조 일치는 fit 공정성 통과와 양립한다.
   b*를 제외하거나 K3를 완화하지 않는다.
   b* 비교에 근거한 score 분기 중단은 Q-35(c) 최소 확인 전까지 유보한다.
   S의 full-K32 승리를 유지하며 R8의 full-covariance 패배 예외에 해당하지 않는다.

2. 검정력: S의 b* 비교는 차이 미검출이지 동등성·유의패배가 아니다.
   MIN_DISC=6은 유의 검정의 최소 가능 쌍 수이지 충분한 검정력이 아니다.
   Q-39(b)의 직접 paired CI는 불확실성 보고용이며 기존 K3를 대체하지 않는다.
   추가 표본이 필요하면 목표 차이·표본 규모·종료 규칙을 별도로 정한다.

3. R1b: 포화가 해석을 제한하지만 원 판정은 미발동으로 유지한다.
   미발동을 "범위 한정 근거 없음"으로 확대하지 않으며 기준도 완화하지 않는다.
   K1 미발동인 현재 R1b를 독립적인 중단 조건으로 사용하지 않는다.

4. b* 과소평가 가능성 [추측]: mean 보존에서 b*가 강화될 가능성을
   score에 불리한 미해결 가설로 보존한다. clip 빈도만으로 손실을 단정하지 않는다.
   Q-40 전에는 이를 확정하거나 배제하지 않는다.
   사실이면 좋은 적합 GMM 대비 M2 진행 논거는 더 약해진다.

[종합 제안: C]
M2 score model 본학습은 보류한다.
score 분기의 중단 및 GMM-prior EP turbo 수신기로의 연구 방향 전환도 확정하지 않는다.
이 결론은 kill test 통과/실패가 아니라 사전 등록에 처방이 없는 조합에 대한
새 운영 결정이다. D-18의 H-GMM baseline·대안 지위는 유지한다.
기여 재구성(D)은 후보로 보존하되 Q-33 확인 전 novelty를 확정하지 않는다.

[진단 우선순위와 경계]
Q-38을 먼저 수행하여 보조 판정의 명세-구현 불일치를 수정한다.
동일 baseline의 literal/literal-mp 및 b*/b*-mp 비교,
원문 K2, R3, R4, R6/R7, R8의 역할을 분리한다.
짝이 없으면 미측정으로 표시한다. 과거 결과 파일과 판독 기록은 보존한다.

Q-40은 Hscore-kron, Hgmm-kron-mp, Hscore-kron-mp의 별도 진단이다.
동일 validation-selected fit/K·trial·seed를 사용하고 추가 EM은 하지 않는다.
clip 효과는 동일 arm eta/mean으로, interface 효과는 동일 fit·clip 아래 비교한다.
새 arm을 기존 (c) 또는 b*에 소급 대입하지 않는다.

Q-37은 기존 H,U/P의 18/21 dB 및 조건부 24 dB 계획을 따른다.
H4의 대응점 없이 새 점의 교차-Tp 포락선을 평가하지 않는다.
기존 S 결정점 재선택이나 R셀·H4셀 확장을 자동 승인하지 않는다.

이 진단 묶음은 S의 검정력, R8의 Q-35(c), Q-33을 모두 해소하지 않는다.
Q-39(b)의 불확실성 보고와 필요시 후속 표본 설계는 별도로 다룬다.
진단 후에도 미결이면 M2 보류를 유지하며 표본·격자·arm을 자동 증량하지 않는다.
추가 확장, K1~K3 재판정 또는 새로운 확인적 진행 gate에는 별도 결정과
그에 맞는 사전 설계가 필요하다. 유리한 진단 결과만으로 K3 통과를 선언하지 않는다.

상태: D-20 제안. 사용자 승인 전이며 관련 Q는 열림 유지.

## [2026-09-18] D-20 검증 (workspace: Claude) — repo 대조, 신규 관찰 2건, 선택지 E 추가

repo 확인: HEAD = 3a13ccfd76edeff4d32edf234a02233d03600858.
GPT가 D-20 제안문에 적은 커밋 해시와 일치한다.
대조 직전 decision_log의 마지막 엔트리는 판독 6 보정이며 D-20은 미push였다.
exp_0925_results.txt는 열지 않았다(세션 7 핸드오프의 판독 재개 금지 준수).
대조 자료는 decision_log 원문, 세션 5/6 핸드오프, exp_0925 소스뿐이다.
GPT 제안문에서 수치 오류나 판정 변조는 발견되지 않았다.

[확인 1] R1b가 K1 발동을 조건으로 하는 구제 분기라는 GPT의 해석은
R1b 원문("K1이 (H,S)에서 발동하더라도 ... 결론은 중단이 아니라 주장 범위 한정")과
일치한다. K1 미발동인 현재 R1b를 독립적 중단 조건으로 쓸 수 없다.
R8 처리, MIN_DISC 해석, b* 과소평가 가설의 불리 방향 보존도 원문과 정합한다.

[확인 2 — D-19 원 논거] D-19 본문(decision_log 277행) 원문:
정확 score는 어떤 학습 score보다 유리한 상한이므로
"정확 score + one-shot 인터페이스(D13+D14)"가
"유한 표본에 적합한 GMM + 정확 site"를 못 이기면 score 분기는 학습 없이 기각된다.
K2 literal이 Hgmm-K32만 지명해 미발동했을 뿐,
이 논거가 요구한 비교는 K3이며 불성립이다.
따라서 "사전 등록에 이 조합의 처방이 없다"는 사실이되,
처방 없음의 방향은 중립이 아니라 score 분기에 불리하다.
이 문장을 D-20 본문에 명시한다.

[확인 3 — 결정 셀 baseline 퇴화] H,S에서 Gaussian(lmmseC_pf) 5.44 dB 대비
Hgmm-K16 7.03, Hgmm-K32 6.65, Hgmm-K64 6.89으로 full-covariance 세 개 모두 나쁘다.
K=32의 우연이 아니며 R3 도입 근거인 8x4 full-covariance EM 과적합과 같은 원인이다.
즉 score가 통과한 유일한 검정(K2 literal)의 상대는 결정 셀에서 퇴화 baseline이고,
실패한 검정(K3)의 상대는 R3가 공정화한 baseline이다. 증거 무게는 비대칭이다.
범위 한정: R,S(4x4)에서는 Gaussian 14.04 대 full-K32 10.63으로 반대 방향이다.
병리는 8x4(N=32, 성분당 N^2=1024 복소 자유도 대 n/K 약 312 표본) 특유이며,
결정 셀이 하필 그 셀이라는 점이 요점이다. 일반화하지 않는다.

[신규 A, 측정] KL(true||fit)이 수신기 성능 순서를 주지 않는다.
적합 품질은 kron 0.198 < full-K32 1.369 < Gaussian 표본 공분산 9.72(307행)인데,
H,S의 SNR@0.1은 kron 3.09 < Gaussian 5.44 < full-K32 6.65 dB로 순서가 뒤집힌다.
Gaussian은 KL이 full-K32보다 약 7배 나쁘지만 SNR@0.1은 1.21 dB 좋다.
정합 관찰: Hscore-K32 6.18 < Hgmm-K32 6.65으로
one-shot 인터페이스가 나쁜 prior에 더 강건하다(판독 4 보정 보강 3의 부호 역전과 동일 방향).
기제는 표본 부족 성분의 과신 responsibility로 추정하며 [추측],
세션 5의 one-shot "확신에 찬 오답" 꼬리와 구조가 같다.
Q-30(b)에 직접 걸린다. 사전 등록 기준이 아니며 K1~K3 판정에 쓰지 않는다.

[신규 B, 측정] H4(Tp=4)에서 prior 모델링 이득이 소멸한다.
판독 4의 H4 표 기준 gain [90% CI], dB:
  Gaussian -> exactEP-true    : S +0.06 [-0.21,+0.33] / P +0.83 / U +0.95
  b* -> exactEP-true          : S -0.00 [-0.13,+0.12] / P +0.12 / U -0.01
  Hscore-exact -> exactEP-true: S -0.10 [-0.22,+0.02] / P +0.03 / U -0.06
prior S에서는 Tp=Nt일 때 Gaussian이 이미 상한에 붙어 있다.
비Gaussian prior의 이득은 Tp=2에 국한된다.
이는 옵션 D("bilinear EP의 정확 mixture site + Tp<Nt regime")의 직접 증거이자
동시에 그 주장 범위의 상한이다. 판독 5(prior P의 R1b)보다 직접적이므로
D를 평가할 때의 1차 근거는 이 표로 한다.
판독 5의 R1b 미발동은 prior P(앙상블 유효 rank 31.6/32, 거의 백색)에서
측정됐고 12 dB의 BLER 0.144 대 요구치 0.118은 근소한 미달이므로,
D의 반증으로도 지지로도 쓰지 않는다.

[신규 C — 판정에 필요한 차이가 현 표본의 분해능 아래에 있다]
판독 4의 직접 paired bootstrap CI:
  b* -> exactEP-true          : +0.28 [-0.17, +0.77]
  Hscore-exact -> exactEP-true : +0.49 [-0.08, +0.96]
  Hscore-exact -> b*           : 점추정 약 +0.21, 직접 CI 미출력(Q-39)
score 분기의 존폐를 가르는 양들이 모두 0을 포함하고, 그 차이에는 CI가 없다.
판독 3 보정도 pooled 37:34(71쌍)가 "60:40 수준의 실제 차이도 검출하지 못한다"고
이미 기록했다. Q-37/38/40 중 어느 것도 이 분해능을 바꾸지 않는다.

[선택지 E — n 증량. A/B/C/D에 빠진 사전 등록 처방]
R6/R7 원문은 검정력 미달 시 "미결(n 증량 또는 SNR 격자 확장)"이다.
sign test 검정력 [정확, 폐형식]:
  n_disc = (z_{a/2}*0.5 + z_b*sqrt(p(1-p)))^2 / (p-0.5)^2
  p=0.6, alpha=.05 양측, power .8 -> n_disc 약 194.
현재 pooled 71쌍, 불일치율 71/1920 약 3.7%
-> 결정 셀 H,S를 n=1920/점(48 chunk, 현재의 3배)로 재실행하면 기대 213쌍.
비용 추정 약 8분(51점 x 640이 19분 8초였으므로 7점 x 3배의 선형 외삽) [추정].
--n은 기존 chunk를 건너뛰고 이어붙는다.
조건(필수): 최종 n과 MDE(60:40)를 실행 전에 확정하고 검정은 한 번만 수행한다.
순차 증량 후 반복 검정은 optional stopping이므로 금지한다.
E는 기존 비교 arm, 검정점, 판정 기준을 바꾸지 않으며 n만 늘린다.

[우선순위 제안]
Q-38(코드, 무료) -> E(결정 셀 n 증량, 사전 설계 동반) -> Q-40 -> Q-35(c) 설계 -> Q-37.
Q-37은 H의 U/P에서 SNR@0.1이 n/a인 문제를 푸는 보고 완결성 항목이며
결정 관련 항목이 아니다. 따라서 후순위로 옮긴다.
E가 K3를 뒤집으면 A를 재심의하고, 유지하면 확인 2의 D-19 원 논거에 따라
B를 기본값으로 재심의한다. 다만 신규 B에 따라
E의 최선의 결과도 Tp=2 밖으로는 확장되지 않는다는 제한을 함께 적용한다.
어느 경우든 Q-33의 2026-10-01 sweep 전에 전환 작업은 착수하지 않는다.

상태: D-20 검증 엔트리. 사용자 승인 전. 관련 Q는 모두 열림 유지.

## [2026-09-18] D-21 (제안) — 결정 셀 (H,S) n 증량: 검정력 확보된 K3 재검정

근거 커밋: 3a13ccfd76edeff4d32edf234a02233d03600858.
D-19 + R1~R8의 비교 arm, 검정 통계량, 판정 기준은 일절 변경하지 않는다.
바뀌는 것은 결정 셀의 n 하나뿐이며, 이는 R6/R7이 명시한
"미달이면 미결(n 증량 또는 SNR 격자 확장)"의 직접 적용이다.
exp_0925_results.txt를 열지 않고 decision_log의 기록만으로 설계했다.

[동기: K3 gate의 검정력]
K2/K3의 판정 형태는 pooled가 아니라 "3개 결정점 중 2개에서 양측 sign test p<.05"이다
(D-19 본문, exp_0925_analysis.py k2_eval의 wins_score).
판독 3의 b* 비교 점별 불일치는 [31,29,11](SNR 3/6/15 dB).
정확 이항으로 계산한 2-of-3 gate의 검정력:
  참 split 60:40 -> 0.024
  참 split 65:35 -> 0.107
즉 현 n에서 K3는 실제 우세가 있어도 성립할 수 없었다.
R6/R7의 MIN_DISC 하한은 코드 주석대로 K2 FIRE(중단 방향)에만 걸린 guard이며
K3 성립(진행 방향)에는 검정력 guard가 없었다. 설계의 비대칭이다.
따라서 판독 3의 "K3 불성립"은 정보량이 거의 없는 결과로 재해석한다.
단, 판독 3의 기록 자체는 변경하지 않는다.

[설계 — 실행 전 확정]
대상: 셀 H, prior S만. 7개 SNR 점 전부(-3/0/3/6/9/12/15 dB).
n: 640 -> 5120 (chunk 40 고정, 점당 128 chunk 중 기존 16개는 skip).
chunk를 키우지 않는다. skip0_n640 파일이 생기면 trial 0-639가 중복 집계된다.
다른 셀과 prior(H4, R, P, U)는 640 그대로 두고 재판정하지 않는다.
MDE: 불일치 split 60:40, alpha=.05 양측, gate 검정력 0.854.
검정은 단 1회. 순차 증량 후 반복 검정(optional stopping)을 금지한다.
이 실행 후 추가 n 증량은 새 결정 없이는 하지 않는다.
본 설계는 55:45를 배제하지 못한다(m=8에서 검정력 0.52). 범위 밖으로 명시한다.

[결정점]
R7 규칙(Hgmm-K32의 BLER@16이 0.1에 |log10| 최근접 3점)을 그대로 재적용한 결과를
1차로 삼는다. n=640에서 선택된 [3,6,15] dB로 고정한 결과를 민감도로 함께 보고한다.
두 집합이 다르면 반드시 그 사실을 보고한다. 유리한 쪽을 사후 선택하지 않는다.
15 dB 점은 불일치율 1.7%로 m=8에서도 점별 검정력 0.44이며
gate 검정력은 사실상 앞의 두 점이 지탱한다. 이 한계를 판독 시 명시한다.

[판독 범위]
새 출력에서 읽는 것은 (H,S)의 K2/K3 세 경로와 그에 딸린 CI뿐이다.
재출력되는 나머지(다른 셀, fit 표, goodput, clip)는 재판독하지 않는다.
판독 1~6의 여섯 판정은 동결 상태를 유지한다.
실행 전 exp_0925_results.txt를 exp_0925_results_n640.txt로 보존한다.

[결과별 사전 처방]
(i) K3 성립(score가 Hgmm-K32와 b*를 각각 2/3점 이상에서 유의 우세):
    M2를 재심의한다. 자동 승인이 아니다. R8에 따라 주장은
    "합성 Kronecker prior 족"으로 한정하고, 판독 4의 H4 표에 따라
    범위를 Tp=2로 한정한다. Q-35(c)는 여전히 남는다.
(ii) K3 불성립(검정력 0.854 하에서):
    "합성 Kronecker prior 족에서 score가 b*를 넘지 못한다"를
    처음으로 정보량 있는 음성 결과로 기록한다.
    단 R8에 따라 b* 비교에 근거한 분기 중단은 확정하지 않으며,
    Q-35(c)가 결정 실험이 된다. B를 Q-35(c) 대기 중의 기본값으로 둔다.
(iii) 결정점 집합이 바뀌거나 R6/R7 하한이 깨지면 판정하지 않고 보고만 한다.

상태: D-21 제안. 실행 전 이 항목을 commit/push한 뒤 실행한다.
