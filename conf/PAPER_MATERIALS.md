...
| `F14a_C2_m3dB_prereg_anchor.*` | **헤드라인 지점 분석, §6q 고정 메뉴 A1~A8.** C2 −3 dB (사전 등록 판정점): (a) 전 arm BLER@16 + Wilson 95%, site ablation·GMM 대조군 나란히, genie 격차 회수율 (b) 반복별 NMSE_H (c) 반복별 BLER (d) 반복별 ν_q 대 동결 격자 (n=8 진단). 가드·복잡도(A4·A8)는 캡션. 생성 `code/figure_f14.py` | `d2sx_N10000_a1.pt` — D2 게이트 행 없음, D1 형제 GC FAIL | **헤드라인 후보 1 (사전 등록 지점)** |
| `F14b_C2_m6dB_max_absolute_gap.*` | 같은 메뉴, C2 −6 dB: **절대 BLER 감소 최대** 0.791→0.601 (544:56, p=2e-101). **사후 선택** — §6p 예측 1(비 최대 −6/−7)이 빗나갔으므로 §6q 규칙 2 로 선택했고 캡션 첫 줄이 그렇게 말한다 | 같음 | **헤드라인 후보 2 (사후)** |
| `F14c_C2_p6dB_max_ratio.*` | 같은 메뉴, C2 +6 dB: **비 최대** 0.0125→0.0039 (3.2×, 블록 오류 69% 제거). **사후 선택**, 캡션 명시 | 같음 | **헤드라인 후보 3 (사후)** |
