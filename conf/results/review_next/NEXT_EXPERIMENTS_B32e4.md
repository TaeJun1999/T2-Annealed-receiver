...
| 항목 | 상태 |
|---|---|
| GPU 큐 `run_b32e4.sh` (kron 2048·1024 재시작, D2·D1 학습, 격자 나머지) | **완료** (09-24 15:33 CDT `B32E4_QUEUE_DONE`; full K=128 이 마지막) — 격자 15개, D2·D1 학습, K=4096 재시작 3개 |
...
| 항목 | 값 (채워질 것) |
|---|---|
| 격자: K 별(계열별) 재시작별 ll_val · n_iter · it_best(상한 도달 여부), 병합 ll_val | [09-24 09:13 CDT] kron K=1024: r0 −10.621 (272 iter, best@270), r1 **−10.559** (341, best@300), r2 −10.941 (171, best@130); 상한 500 미도달; 병합 = r1, ll_val −10.559, ll_test −10.230, sec 합 93,569 (fit_gpu --merge, CPU). [09-24 11:19 CDT] kron K=2048: r1 −8.023 (231, best@190), r0·r2 진행 중. 나머지 K: 진행 중. **[09-24 15:55 CDT] 격자 15개 완료** — 병합 ll_val (선택 재시작, n_iter·best@; `results/gmm_fits_D2_B32e4/*.npz` 에서 읽음). **kron** (κ=0, 재시작 3): 16 −38.811 (r2, 176·170) / 32 −32.909 (r0, 191·190) / 64 −27.546 (r0, 198·190) / 128 −22.473 (r2, 235·200) / 256 −18.278 (r2, 210·180) / 512 −13.935 (r1, 325·320) / 1024 −10.559 (r1, 341·300) / 2048 **−7.793** (r2, 471·430; r0 −7.981 271·230, r1 −8.023 231·190) / 4096 **−5.942** (r1, 221·180; r0 −5.964 181·140, r2 −5.989 241·200) — 상한 500 도달 없음. **full** (κ ∈ {0,16,64,256} × 재시작 3, 모든 K 에서 κ=0 선택): 16 −38.443 (r1, 151·150) / 32 −31.250 (r0, 342·340) / 64 −25.171 (r2, 371·330) / 128 −19.636 (r0, 361·320) / 256 −14.648 (r2, 500·490, **상한 도달**) / 512 −11.437 (r1, 500·490, **상한 도달**). **재시드**(성분 고갈 nk < 4): kron K=4096 에서만 재시작별 2421 / 3170 / 3311 회(N′/K ≈ 78), 나머지 K 는 전부 0 — 희소 검증(K=1024)은 재시드 0 이라 재시드 경로를 직접 겪지 않았으나, 재시드 판단(nk, 두 경로 공통 — 희소 경로는 nk 를 건드리지 않는다)과 RNG 소비는 공통 코드. **구현 경로**(kron M-단계만 해당; full 은 무관): 정확 = kron 256·1024 전 재시작·2048 r0/r1; 일괄(batched, 96ca6581, `results/b32e4_batched_check*.txt` PASS) = kron 16·32·64·128·512 전 재시작·2048 r2; 희소 tol 1e-12 = kron 4096 전 재시작. 재시작 후보 누락·재실행 없음 |
...
| 최종 b\*: 계열·K·ll_val, 내부 여부(또는 격자 끝 캐비엇) | [09-24 14:34 CDT] **사용자 결정: K=4096 에서 멈춤** (DECISIONS 같은 시각) → b\* = kron K=4096 병합값(재시작 1·2 완료 후 확정), **격자 끝 캐비엇**(K=2048 대비 약 +2 nat 상승 중에 멈춤; 4e4 선례와 같은 처리). 확정 ll_val 은 병합 후 여기에 추가. **[09-24 15:55 CDT] 확정: b\* = kron K=4096, ll_val −5.942083265612076** (재시작 1, n_iter 221, best@180, ll_test −5.781; 수용 검사의 `ll_val|kron` 대조값). **격자 끝** — K=2048 병합(−7.793) 대비 +1.851 nat 로 아직 오르는 중에 멈춤(§1 반복 규칙상 다음은 K=8192, 사용자가 거부). full 계열 최고(K=512 −11.437, 상한 도달)는 b\* 보다 5.5 nat 낮다 |
