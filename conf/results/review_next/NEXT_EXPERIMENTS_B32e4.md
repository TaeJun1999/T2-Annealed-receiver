...
| 최종 b\*: 계열·K·ll_val, 내부 여부(또는 격자 끝 캐비엇) | [09-24 14:34 CDT] **사용자 결정: K=4096 에서 멈춤** (DECISIONS 같은 시각) → b\* = kron K=4096 병합값(재시작 1·2 완료 후 확정), **격자 끝 캐비엇**(K=2048 대비 약 +2 nat 상승 중에 멈춤; 4e4 선례와 같은 처리). 확정 ll_val 은 병합 후 여기에 추가 |
| D2 체크포인트: last / best 의 sha256[:16]·epoch·best_epoch·stopped_by·aborted·init sha256[:16]·`torch.initial_seed`; best.epoch == last.best_epoch 확인 | [09-24 13:24 CDT] `d2sx_N320000_a1.pt` (last) sha256[:16] f2f1eebc9894c773, epoch 986, best_epoch 966, stopped_by=patience, aborted=False (로그 '# done'), rung D2SX320000 a1; `d2sx_N320000_a1_best.pt` sha 035744cbe955984d, epoch 966 = last.best_epoch ✓, role best. 학습 09-24 01:21~12:43 CDT, 40,954 s, 41.5 s/epoch, 발산 없음(§3d 미적용). init sha·`torch.initial_seed`: **기록 없음** — `run_d2_sx.py` 는 남기지 않는다(N1: 초기값 비고정). 끝의 GB′ 단계는 예상대로 실패(rc=1, 태그 없는 fits 디렉터리) |
